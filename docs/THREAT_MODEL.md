...
- build-once candidate artifact construction, archive/privacy inspection, artifact-only installation,
  finite release matrices, hashes, and review evidence;
- dormant manual publication validation and separation of OIDC permission from build/test jobs;
...
| Proposed corrective artifacts | Bind one source commit, version, wheel, sdist, archive inventory, hashes, and completed matrix evidence without treating a partial run as release-ready. |
| Publication authority | Keep artifact construction separate from the protected environment and OIDC job; require exact tag/commit/version/hash evidence and explicit later human authorization. |
...
| Candidate artifacts include private, generated, or unexpected material | Hatch uses an explicit sdist allowlist. The candidate verifier rejects unsafe archive paths, non-regular sdist members, forbidden directories/file types, credential shapes, runner paths, and recursive privacy canaries before writing build evidence. | Pattern scans cannot prove that all sensitive or malicious material is absent. A compromised builder or dependency can subvert process-level checks; independent archive review remains required. |
| A partial or failed candidate run is mistaken for release readiness | Build evidence says only `build-verified`. A separate final manifest is created only by a job that depends on wheel/sdist installs plus every base, WP7C, and WP7D matrix job; it says `ready-for-independent-review` and explicitly records `publication_authorized: false`. | GitHub Actions, runner images, artifact retention, service availability, and log completeness remain external. A green run is evidence for one exact run, not a production-safety or publication guarantee. |
| A publication job rebuilds or substitutes artifacts | The dormant workflow has no build command, validates all dispatch inputs before checkout/download, downloads one positive candidate run twice, requires exact SHA-256 inputs and evidence, validates the annotated tag and commit, and rechecks bytes immediately before the PyPI action. | GitHub artifact service, Actions dependencies, account compromise, and malicious repository administration remain supply-chain risks. |
| A modified publication workflow reaches PyPI authority from an arbitrary branch | Dispatch must originate from the exact `publish-pypi.yml@refs/tags/v0.1.1` workflow ref and reviewed commit. The `pypi` environment must prevent self-review and allow only the exact `v0.1.1` tag through its deployment policy. | Tag creation, environment configuration, reviewer eligibility, repository administration, and external trust remain human-controlled boundaries. A single maintainer cannot both dispatch and approve. |
| Build or test code obtains a PyPI credential | The candidate workflow has repository read permission only and no `id-token`. Only the final `pypi` environment job has `id-token: write`; no stored publication secret, username, password, or TestPyPI target exists. The exact external tuple is repository `Agenvana/RAGLeakGuard`, workflow `publish-pypi.yml`, environment `pypi`. | OIDC is powerful publication authority once external trust is configured. Environment protection and the PyPI Trusted Publisher are external administrative state and require independent review. |

...
| Incomplete scan reported as success | Unsupported sources and missing Chroma paths exit non-zero. | Unsupported locales silently no-op; `scan` exits 0 for missing detection/model dependencies; empty scans return before writing a report. Fail-closed typed errors are **planned**. |
| False negatives mistaken for absence of sensitive data | Reports state that detection is best-effort. Locale pack is opt-in. | No detector is complete; current CLI help overnames unimplemented locales. Operators must not use a clean scan as proof of safety. |

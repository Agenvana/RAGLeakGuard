...
| A publication job rebuilds or substitutes artifacts | The dormant workflow has no build command, downloads a named candidate run twice, requires exact SHA-256 inputs and evidence, validates the annotated tag and commit, and rechecks bytes immediately before the PyPI action. | GitHub artifact service, Actions dependencies, account compromise, and malicious repository administration remain supply-chain risks. |
| Build or test code obtains a PyPI credential | The candidate workflow has repository read permission only and no `id-token`. Only the final `pypi` environment job has `id-token: write`; no stored publication secret, username, password, or TestPyPI target exists. | OIDC is powerful publication authority once external trust is configured. Environment protection and the PyPI Trusted Publisher are external administrative state and require independent review. |
